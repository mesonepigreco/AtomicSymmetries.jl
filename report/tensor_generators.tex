...
(ii)~building the Reynolds matrix $P$ requires $|G_{\vT}|$ applications of $\hat{g}$ on $D_{\mathcal{S}}$ basis vectors, each costing $O(k \cdot d^{2k})$, plus the projection back onto the basis: $O(D_{\mathcal{S}}^2 \cdot |G_{\vT}| \cdot d^k)$;
(iii)~SVD of the $D_{\mathcal{S}} \times D_{\mathcal{S}}$ matrix: $O(D_{\mathcal{S}}^3)$;
(iv)~orbit propagation: $O(|G| \cdot d^{2k})$ per invariant vector.
